...
public class SearchViewImpl extends BaseViewImpl<SearchPresenter> implements SearchView {
	...
	@UiHandler("submit_button")
	void onSubmitButtonClick(ClickEvent evt) {
		String deptID = dept_box.getText();
		if (users_button.getValue()) {
			presenter.sendSearchUsers(deptID);
		}
		else if (courses_button.getValue()) {
			presenter.sendSearchCourses(deptID);
		}
